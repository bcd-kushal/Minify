                ...
                <form action={handleSignIn}>
                    <div className="grid w-full items-center gap-4">
                        <div className="flex flex-col space-y-1.5">
                            ...
                            <Input id="name" name="signin_name" type="text" placeholder="" />
                        </div>
                        <div className="flex flex-col space-y-1.5 mt-1">
                            <Label htmlFor="framework" className="mb-2">Password</Label>
                            <Input id="password" name="signin_pass" type="password" placeholder="" />
                        </div>
                    </div>
                    <Button type="submit" className="mt-8" disabled={loading}><ReloadIcon className={loading ? "mr-2 h-4 w-4 animate-spin" : "hidden"} />Signin</Button>
                </form>
            </CardContent>
        </Card>
    )
...
export function RegistrationForm({ props }: { props?: CardProps }) {
    ...
    const handleRegistration = async (formData: FormData) => {
        ...
        setTimeout(() => {
            if (formError.email || formError.password || formError.username)
                ...
                // if they dont, redirect to register page
                // if creds match go to dashboard
                if (credentials.username === "winterr" && credentials.email === "am.kushal02@gmail.com")
                    toast({
                        title: "This user exists",
                        description: "You are already a registered user",
    ...
    const handleUsername = (inputValue: React.KeyboardEvent<HTMLElement>) => {
        const entry = (inputValue.target as HTMLInputElement).value
        setFormError(prev => ({ ...prev, username: entry === "" ? false : !REGEX_TEST.NAME.test(entry) }))
    }
    const handleEmail = (inputValue: React.KeyboardEvent<HTMLElement>) => {
        const entry = (inputValue.target as HTMLInputElement).value
        setFormError(prev => ({ ...prev, email: entry === "" ? false : !REGEX_TEST.EMAIL.test(entry) }))
    }
    const handlePassword = (inputValue: React.KeyboardEvent<HTMLElement>) => {
        const entry = (inputValue.target as HTMLInputElement).value
        setFormError(prev => ({ ...prev, password: entry === "" ? false : !REGEX_TEST.PASSWORD.test(entry) }))
    }
    return (
                ...
                <form action={handleRegistration}>
                    <div className="grid w-full items-center gap-3">
                        <div className="flex flex-col space-y-1.5">
                            ...
                            <Input id="register_name" name="register_name" type="text" placeholder="" onKeyUp={handleUsername} />
                        ...
                        <div className="flex flex-col space-y-1.5">
                            ...
                            <Input id="register_email" name="register_email" type="email" placeholder="" onKeyUp={handleEmail} />
                        ...
                        <div className="flex flex-col space-y-1.5 mt-1">
                            ...
                            <Input id="password" name="register_password" type="password" placeholder="" onKeyUp={handlePassword} />
                            <div className="text-xs text-red-400">{formError.password ? (<> Minimum length 8. Should contain atleast: <br/>
                                                                <span className="text-[#624]">• atleast one upper-case letter</span> <br/>
                                                                <span className={``}>• atleast one lower-case letter</span> <br/>
                                                                <span className={``}>• atleast one digit(0-9)</span> <br/>
                                                                <span className={``}>• atleast one special character from @#$%^&+=_</span> </>) 
                                                        : ""}
                            </div>
                        </div>
                        <div className="flex flex-col space-y-1.5 mt-1">
                    ...
                    <CardFooter className="flex justify-between px-0 mt-8 pb-2 items-center">
                        <Button type="submit" className="mt-8" disabled={loadingRegister}><ReloadIcon className={loadingRegister ? "mr-2 h-4 w-4 animate-spin" : "hidden"} />Register</Button>
