import { Menubar, MenubarCheckboxItem, MenubarContent, MenubarItem, MenubarMenu, MenubarRadioGroup, MenubarRadioItem, MenubarSeparator, MenubarShortcut, MenubarSub, MenubarSubContent, MenubarSubTrigger, MenubarTrigger } from "@/components/ui/menubar"
  ...
  export function MenubarComponent({ props }: { props?:{ cph?:string, css_cph?:string, cpw?:string, css_cpw?:string } }) {
    const tlw = { h:props?.cph||"[100%]", w:props?.cpw||"[100%]" }
    const css = { h:props?.css_cph||"100%", w:props?.css_cpw||"100%" }
    return (
      <Menubar className={`h-${tlw.h} w-${tlw.w}`} style={{width:css.w}}>
        <MenubarMenu>
